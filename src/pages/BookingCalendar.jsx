...
export default function BookingCalendar() {
  ...
  const { data: bookingProfile } = useQuery({
    ...
    queryFn: async () => {
      ...
      const profiles = await base44.entities.BookingProfile.filter({ user_id: u.id });
  ...
  const { data: allBookings = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [hosting, attending] = await Promise.all([
        base44.entities.Booking.filter({ professional_id: u.id }, '-scheduled_at', 50),
        base44.entities.Booking.filter({ client_id: u.id }, '-scheduled_at', 50),
      ]);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      if (bookingProfile) {
        await base44.entities.BookingProfile.update(bookingProfile.id, profileForm);
      } else {
        await base44.entities.BookingProfile.create({ ...profileForm, user_id: user.id });
      }
