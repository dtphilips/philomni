  ...
  const generate = async () => {
    ...
    try {
      const res = await base44.integrations.Core.InvokeLLM({
        prompt: `Generate 3 ${tone.toLowerCase()} social media captions for ${platform} about: "${topic}".
${hashtags ? 'Include relevant hashtags.' : ''}
Return as JSON array: [{"caption": "...", "hashtags": [...]}]`,
        response_json_schema: {
          type: 'object',
          properties: { captions: { type: 'array', items: { type: 'object', properties: { caption: { type: 'string' }, hashtags: { type: 'array', items: { type: 'string' } } } } } }
        }
      });
      const parsed = typeof res === 'string' ? JSON.parse(res) : res;
      setResults(parsed.captions || []);
    } catch (_) {
      // Fallback mock
      setResults([
...
function ScriptWriter({ user }) {
  ...
  const generate = async () => {
    ...
    try {
      const res = await base44.integrations.Core.InvokeLLM({
        prompt: `Write a ${duration}-second ${type.toLowerCase()} video script about: "${topic}".
Format with clear sections: HOOK, BODY, CTA.
Make it engaging, natural-sounding, and optimised for ${duration}s runtime.`,
      });
      setScript(typeof res === 'string' ? res : res?.response || JSON.stringify(res));
    } catch (_) {
...
function ContentPlanner({ user }) {
  ...
  const generate = async () => {
    ...
    try {
      const res = await base44.integrations.Core.InvokeLLM({
        prompt: `Create a 30-day social media content plan for a ${niche} creator whose goal is to "${goal}".
Return exactly 30 items as JSON array:
[{"day": 1, "theme": "...", "content_type": "...", "topic": "...", "caption_hook": "..."}]
content_type can be: Post, Reel, Story, Carousel, Live, Poll`,
        response_json_schema: {
          type: 'object',
          properties: { days: { type: 'array', items: { type: 'object', properties: { day: { type: 'number' }, theme: { type: 'string' }, content_type: { type: 'string' }, topic: { type: 'string' }, caption_hook: { type: 'string' } } } } }
        }
      });
      const parsed = typeof res === 'string' ? JSON.parse(res) : res;
      setPlan(parsed.days || []);
