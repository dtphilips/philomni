...
export default function Login() {
  ...
  useEffect(() => {
    ...
    // Navigate once Supabase confirms the sign-in — avoids a race between
    // manual navigate() and the auth state update in AuthContext.
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            className="w-full h-11 rounded-xl flex items-center justify-center gap-2 text-sm select-none"
            style={{
          ...
          >
            <svg viewBox="0 0 24 24" className="w-4 h-4 opacity-40 flex-shrink-0">
            ...
            <span
              ...
              style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.3)', fontSize: '10px' }}
            >
              Coming soon
            </span>
          </div>
      ...
      <style>{`
        ...
        .auth-input:focus { border-color: rgba(124,58,237,0.55); }
        .auth-btn-secondary {
          width: 100%;
          height: 44px;
          border-radius: 0.75rem;
        ...
        .auth-btn-primary:hover:not(:disabled) { opacity: 0.9; box-shadow: 0 0 28px rgba(109,40,217,0.45); }
...
function DarkField({ label, right, children }) {
  return (
...
function PrimaryButton({ children, className = '', ...props }) {
  return (
    <button className={`auth-btn-primary ${className}`} {...props}>
      {children}
    </button>
  );
}
