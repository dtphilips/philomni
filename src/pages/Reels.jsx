...
function ReelCard({ reel, isActive, currentUser }) {
  ...
  const handleLike = async () => {
    ...
    try {
      if (!liked) {
        await base44.entities.Like.create({ post_id: reel.id, user_id: currentUser?.id });
      }
    } catch (_) {}
  };
  ...
  const handleComment = async () => {
    ...
    try {
      await base44.entities.Comment.create({ post_id: reel.id, content: comment });
      setComment('');
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await base44.entities.Post.create({
        content: caption,
        post_type: 'reel',
        video_url: file.type.startsWith('video/') ? uploaded.file_url : null,
        media_url: file.type.startsWith('image/') ? uploaded.file_url : null,
        visibility: 'public',
      });
...
export default function Reels() {
  ...
  const { data: reels = [], isLoading } = useQuery({
    queryKey: ['reels'],
    queryFn: () => base44.entities.Post.filter({ post_type: 'reel', visibility: 'public' }),
  });
