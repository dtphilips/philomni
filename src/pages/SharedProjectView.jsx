...
export default function SharedProjectView() {
  ...
  useEffect(() => {
    const loadProject = async () => {
      setLoading(true);
      const proj = await base44.entities.SharedProject.read(projectId);
      if (proj) {
        setProject(proj);
        // Increment view count
        await base44.entities.SharedProject.update(projectId, { view_count: (proj.view_count || 0) + 1 });
        // Load comments
        const cmts = await base44.entities.ProjectComment.filter({ project_id: projectId }, '-created_date', 100);
        setComments(cmts);
        // Load ratings
        const rtgs = await base44.entities.TemplateRating.filter({ project_id: projectId });
        setRatings(rtgs);
        if (rtgs.length > 0) {
  ...
  const handleFork = async () => {
    ...
    const forkedProject = await base44.entities.SharedProject.create({
      owner_id: user.id,
    ...
    await base44.entities.SharedProject.update(projectId, { fork_count: (project.fork_count || 0) + 1 });
  ...
  const handleRate = async (rating) => {
    ...
    if (userRating) {
      ...
      await base44.entities.TemplateRating.update(userRating.id, { rating });
      setUserRating({ ...userRating, rating });
    } else {
      // Create new rating
      const newRating = await base44.entities.TemplateRating.create({
        project_id: projectId,
        user_id: user.id,
        rating,
      });
