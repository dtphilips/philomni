...
export default function VideoCaptions() {
  ...
  const { data: draft } = useQuery({
    ...
    queryFn: async () => {
      ...
      const drafts = await supabase.from('video_drafts').select('*') /* TODO filter: {} */;
  ...
  const { data: captions } = useQuery({
    ...
    queryFn: async () => {
      if (!draftId) return null;
      const result = (await supabase.from('video_captions').select('*').eq('draft_id', draftId)).data ?? [];
      return result[0];
    },
    enabled: !!draftId
  });
  ...
  const handleGenerateCaptions = async () => {
    ...
    try {
      const llmRes = await fetch('/api/llm', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: `Generate video captions/subtitles for a video titled: "${draft.title ?? 'Untitled'}". Return 5 caption segments with timestamps.` }) });
      const llmData = await llmRes.json();
      const segments = [{ id: 1, start: 0, end: 5, text: llmData.result?.slice(0, 80) ?? 'Caption 1' }];
      await supabase.from('video_captions').insert({ draft_id: draftId, captions: segments, language: 'en' });
  ...
  const handleSaveCaption = async (captionId) => {
    ...
    try {
      ...
      (await supabase.from('video_captions').update(updated).eq('id', captions.id).select().single()).data;
      queryClient.invalidateQueries({ queryKey: ['captions', draftId] });
      setEditingId(null);
