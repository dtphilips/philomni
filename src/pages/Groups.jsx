...
export default function Groups() {
  ...
  const { data: allGroups = [] } = useQuery({
    queryKey: ['groups'],
    queryFn: async () => { const { data } = await supabase.from('groups').select('*').eq('is_active', true).order('created_at', { ascending: false }); return data ?? []; },
  });
  ...
  const { data: userGroups = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const members = (await supabase.from('group_members').select('*').eq('user_id', currentUser.id)).data ?? [];
      const groupIds = members.map(m => m.group_id);
      const groups = await Promise.all(
        groupIds.map(id => supabase.from('groups').select('*') /* TODO filter: { id } */)
      );
      return groups.flat();
    },
    enabled: !!currentUser,
