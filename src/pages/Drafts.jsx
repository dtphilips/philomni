...
export default function Drafts() {
  ...
  const { data: drafts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return supabase.from('video_drafts').select('*') /* TODO filter: { creator_id: user.id }, '-updated_date', 100 */;
    }
  });
  ...
  const handleCreateDraft = async () => {
    ...
    try {
      const user = user /* useAuth() */;
      (await supabase.from('video_drafts').insert({
        creator_id: user.id,
        title: newDraftTitle,
        status: 'editing'
      }).select().single()).data;
  ...
  const handleDeleteDraft = async (id) => {
    try {
      await supabase.from('video_drafts').delete().eq('id', id);
      queryClient.invalidateQueries({ queryKey: ['drafts'] });
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      (await supabase.from('video_drafts').update({ status: newStatus }).eq('id', id).select().single()).data;
      queryClient.invalidateQueries({ queryKey: ['drafts'] });
